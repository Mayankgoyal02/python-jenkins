...
# Run the test and capture the response
response=$(curl -sk "$RUN_TEST_URL" \
 -X POST \
 -H 'Content-Type: application/json' \
 --user "$USERNAME:$API_KEY")
 
# Extract the test ID from the response using grep and awk
test_id=$(echo "$response" | grep -o '"id":[^,]*' | awk -F':' '{print $2}' | tr -d '"')
 
if [ -n "$test_id" ]; then
 # Print the BlazeMeter test dashboard URL
 echo "BlazeMeter test dashboard URL: https://a.blazemeter.com/app/tests/$test_id"
else
 echo "Error: Failed to get test ID from BlazeMeter response."
fi
